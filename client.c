...
struct PS_INFO {
	...
	char user[20];
	char run_time[20];
	char command[100];	//topÀÌ¶û ´Ù¸£°Ô ³ª¿Â´Ù 
};	//psÀÇ °á°ú¸¦ ´ãÀ» ±¸Á¶Ã¼
...
void kill_ps(int index, char psname[], int s, struct PROCESS_INFO *p) {
	...
	if (index == -1) {//pid°¡ ¾Æ´Ï¶ó psnameÀ¸·Î ÇÒ·Á°í ÇÒ·Á¸é¤¤
...
struct PROCESS_INFO* make_top_info(char filename[20]) {
	...
	while (1) {
		...
		if (isdigit(line[4]) != 0) { //%5d·Î µÇ¾î ÀÖ¾î¼­ 5¹øÂ° Ä­ÀÌ ¼ýÀÚÀÎÁö È®ÀÎ
...
struct PS_INFO* make_ps_info(char* filename) {
	...
	fgets(line, 2000, fp);	//Ã¹ ÇÑÁÙÀº ½ºÅµ
	while (1) {
